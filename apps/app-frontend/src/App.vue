...
import { check_reachable } from '@/helpers/auth'
...
const messages = defineMessages({
	...
	signInToModrinthAccount: {
		...
		defaultMessage: 'Sign into Rubirinth',
	},
	loadingProfile: {
...
async function setupApp() {
	...
	if (locale) {
		await traceStartupStep('Apply startup locale', () => setLocale(locale || 'ru-RU'))
	} else {
		await traceStartupStep('Apply startup locale', () => setLocale('ru-RU'))
	}
	...
	appSettings.discordRpc = discord_rpc ?? true
	if (os.value !== 'MacOS') {
